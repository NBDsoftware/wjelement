...
static void storeErrorMessage(Node * node, char * format, ...){
	...
	NDE_setErrorMessage(node, buffer);
}

static XplBool SchemaValidate(WJElement schema, WJElement document,
							Node * errorNode,  WJEErrCB err, WJESchemaLoadCB loadcb,
	...
	if(schema) {
		...
		if(loadcb && (str = WJEString(schema, "[\"$ref\"]", WJE_GET, NULL))) {
			...
			fail = SchemaValidate(sub, document, errorNode, err, loadcb, freecb, client,
								  name);
			if(freecb) {
				freecb(sub, client);
			}
		...
		if((sub = WJEObject(schema, "extends", WJE_GET))) {
			/* inline schema object */
			anyFail = anyFail || !SchemaValidate(sub, document, errorNode, err, loadcb,
												 freecb, client, name);
		} else if(loadcb &&
		...
		if(!str && !sub) {
			...
			while(loadcb && (str = _WJEString(schema, "extends[]", WJE_GET,
											  &last, NULL))) {
				if((sub = loadcb(str, client, __FILE__, __LINE__))) {
					anyFail = anyFail || !SchemaValidate(sub, document, errorNode, err,
														 loadcb, freecb,
														client, name);
				}
				if(sub && freecb) {
		   ...
		   !stricmp(memb->name, "disallow")) {
			if(memb->type == WJR_TYPE_ARRAY) {
				...
				while((arr = WJEGet(memb, "[]", arr))) {
					if(arr->type == WJR_TYPE_STRING) {
						...
						if(SchemaValidate(arr, document, NDE_createErrorNode(errorNode), err,
				...
				if(!SchemaValidate(memb, document, NDE_createErrorNode(errorNode), err,
								   loadcb, freecb, client, name)) {
					fail = TRUE;
				}
			}
			...
			if(err && fail) {
				if(memb->type == WJR_TYPE_OBJECT) {
					if(name) {
						...
						storeErrorMessage(errorNode, "%s failed validation\n", name);
					} else if(document) {
						//err(client, "%s failed validation", document->name);
						storeErrorMessage(errorNode, "%s failed validation\n", document->name);
					} else {
						//err(client, "failed validation");
						storeErrorMessage(errorNode, "failed validation\n");
					}
				} else {
					if(name) {
						//err(client, "%s is of incorrect type", name);
						storeErrorMessage(errorNode, "%s is of incorrect type\n", name);
					} else if(document) {
						//err(client, "%s is of incorrect type", document->name);
						storeErrorMessage(errorNode, "%s is of incorrect type\n", document->name);
					} else {
						//err(client, "incorrect type");
						storeErrorMessage(errorNode, "incorrect type\n");
					}
				}
			}
			...
			if(document && memb->type == WJR_TYPE_OBJECT) {
				...
				while((arr = WJEGet(memb, "[]", arr))) {
					...
					if(!SchemaValidate(arr, data, NDE_createErrorNode(errorNode), err, loadcb, freecb, client,
									   arr->name)) {
						fail = TRUE;
						if(err) {
					...
					if(!regcomp(&preg, arr->name, REG_EXTENDED | REG_NOSUB)) {
						...
						while((data = WJEGet(document, "[]", data))) {
							if(!regexec(&preg, data->name, 0, NULL, 0)) {
								...
								if(!SchemaValidate(arr, data, NDE_createErrorNode(errorNode), err,
												   loadcb, freecb,
												   client, arr->name)) {
									fail = TRUE;
									if(err) {
...
//											arr->name);
						...
						if(err) {
							...
							storeErrorMessage(errorNode, "%s failed to build regex\n", arr->name);
						}
					}
				}
			}
...
//									name, data->name);
								storeErrorMessage(errorNode, "%s: additional property '%s' found.\n", name, data->name);
							}
						}
					}
				} else if(memb->type == WJR_TYPE_OBJECT) {
					...
					while((data = WJEGet(document, "[]", data))) {
						if(!WJEChild(sub, data->name, WJE_GET)) {
							if(!SchemaValidate(memb, data, NDE_createErrorNode(errorNode), err, loadcb, freecb,
											   client, data->name)) {
								fail = TRUE;
								if(err) {
//									err(client,
...
//										name, data->name);
									storeErrorMessage(errorNode, "%s: extra property '%s' not valid.\n",
											name, data->name);
								}
							}
						}
			...
			if(document && document->type == WJR_TYPE_ARRAY) {
				if(memb->type == WJR_TYPE_OBJECT) {
					...
					while((arr = WJEGet(document, "[]", arr))) {
						...
						if(!SchemaValidate(memb, arr, NDE_createErrorNode(errorNode), err, loadcb, freecb,
										   client, str)) {
							fail = TRUE;
							if(err) {
					...
					while((arr = WJEGet(memb, "[]", arr))) {
						...
						MemAsprintf(&str, "%s[%d]", name, val);
						if(!SchemaValidate(arr, data, NDE_createErrorNode(errorNode), err, loadcb, freecb,
										   client, str)) {
							fail = TRUE;
							if(err) {
//								err(client, "%s failed tuple type vaidation.",
//									str);
								storeErrorMessage(errorNode, "%s failed tuple type validation.\n", str);
							}
						}
						MemFree(str);
						str = NULL;
...
//							document->count - data->count);
						storeErrorMessage(errorNode, "%s: %d additional item(s) found\n", name,
								document->count - data->count);
					}
				} else if(memb->type == WJR_TYPE_OBJECT) {
					val = 0;
...
//									str);
								storeErrorMessage(errorNode, "additional item %s failed validation.", str);
							}
						}
						MemFree(str);
						str = NULL;
			...
			if(fail && err) {
				...
				storeErrorMessage(errorNode, "required item '%s' not found.\n", name);
			}
			anyFail = anyFail || fail;
			...
			if(document && memb->type == WJR_TYPE_OBJECT) {
				...
				while((arr = WJEGet(memb, "[]", arr))) {
					if(arr->type == WJR_TYPE_OBJECT) {
						if(!SchemaValidate(arr, document, NDE_createErrorNode(errorNode), err, loadcb, freecb,
										   client, name)) {
							fail = TRUE;
							if(err) {
//								err(client, "broken dependency schema: %s",
//									arr->name);
								storeErrorMessage(errorNode, "broken dependency schema: %s\n", arr->name);
							}
						}
...
//									name, str);
								storeErrorMessage(errorNode, "%s: dependency '%s' not found\n", name, str);
							}
						}
...
//						name, dval, dnum);
					storeErrorMessage(errorNode, "%s: minimum value (%lf) not met (%lf).\n",
							name, dval, dnum);
				}
				anyFail = anyFail || fail;
			}
...
//						name, dval, dnum);
					storeErrorMessage(errorNode, "%s: maximum value (%lf) not met (%lf).\n",
							name, dval, dnum);
				}
				anyFail = anyFail || fail;
			}
...
//						name, val, document->count);
					storeErrorMessage(errorNode, "%s: minimum items (%d) not met (%d).\n", name, val, document->count);
				}
				anyFail = anyFail || fail;
			}
...
//						name, val, document->count);
					storeErrorMessage(errorNode, "%s: maximum items (%d) exceeded (%d).\n",
							name, val, document->count);
				}
				anyFail = anyFail || fail;
			}
...
//						name, val, num);
					storeErrorMessage(errorNode, "%s: minimum length (%d) not met (%d).\n",
							name, val, num);
				}
				anyFail = anyFail || fail;
			}
...
//						name, val, num);
					storeErrorMessage(errorNode,"%s: maximum length (%d) exceeded (%d).\n",
							name, val, num);
				}
				anyFail = anyFail || fail;
			}
...
//									name, i, name, num);
								storeErrorMessage(errorNode,"%s[%d] identical to %s[%d].\n",
										name, i, name, num);
							}
						}
						MemFree(str);
...
//						err(client, "%s: non-unique items found.", name);
						storeErrorMessage(errorNode,"%s: non-unique items found.\n");
					}
				}
			}
			anyFail = anyFail || fail;
...
//						name, WJEString(document, NULL, WJE_GET, ""), str);
					storeErrorMessage(errorNode,"%s: '%s' does not match '%s' format.\n",
							name, WJEString(document, NULL, WJE_GET, ""), str);
				}
				anyFail = anyFail || fail;
			}
			...
			if(document && memb->type == WJR_TYPE_ARRAY) {
				...
				if(!val) {
					...
					if(err) {
						...
						storeErrorMessage(errorNode,"%s: no enum value found.\n", name);
					}
				}
				anyFail = anyFail || fail;
			}
...
//						name, dnum, dval);
					storeErrorMessage(errorNode,"%s: %lf not divisible evenly by %lf.\n",
							name, dnum, dval);
				}
				anyFail = anyFail || fail;
			}
...
EXPORT XplBool WJESchemaValidate(WJElement schema, WJElement document,
								 ...
								 WJESchemaFreeCB freecb, void *client) {
	Node * errorNode = NDE_createErrorNode(NULL);
	XplBool ret = SchemaValidate(schema, document, errorNode, err, loadcb, freecb, client,
						  "(root)");
	NDE_dump(errorNode);
	NDE_clearNode(errorNode);
	return ret;
}
